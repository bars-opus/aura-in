-- ============================================================
-- Patch: pre-selected workers use direct workers lookup
-- Deployed: 2026-05-25
--
-- When p_selected_worker_ids is provided, the previous version
-- still called get_available_workers() with those IDs. Despite
-- the IDs being valid (confirmed by workersForSlot debug prints),
-- that inner call returned 0 rows for unknown reasons, making
-- the slot condition false and the function return nothing.
--
-- Fix: when workers are pre-selected, query the workers table
-- directly inside generate_available_slots — no booking overlap
-- or unavailability checks. Conflict detection is enforced at
-- booking-creation time by create_booking_with_conflict_check.
-- The slot screen only needs to show valid time windows.
-- ============================================================
  ...
  FOREACH v_svc_id IN ARRAY p_service_ids LOOP
    ...
    WHILE v_t::TIME <= v_closes - (v_dur_min || ' minutes')::INTERVAL LOOP
      ...
      IF v_use_selected THEN
        -- Workers were pre-chosen by the user: include them directly.
        -- No booking/unavailability check here; create_booking_with_conflict_check
        -- enforces actual conflicts at booking time.
      ...
      ELSE
        -- No worker pre-selected: get available workers from slot assignments.
