-- Add service_address to booking_simple.
--
-- bookings.address stores the client's home-service address for freelancer
-- bookings (set by create_booking_with_conflict_check via p_service_address).
-- The view previously omitted it, so RoutePreviewWidget in BookingShopInfoCard
-- fell back to the shop address — which for freelancers is the freelancer's
-- name, not the client's location.
...
SELECT
  ...
  b.client_address              AS service_address,
  b.client_address_lat          AS service_latitude,
  b.client_address_lng          AS service_longitude,
